...
(b) Create a new column that reflects each flights departure date in the following format: "Day of the week, Month day, Year" (i.e., "Monday, January 8, 2013"). Then select only that new column, and show the first three rows. To select the first three rows, use the function `head` ***Your final output should be a printed dataframe with one column and three rows.***
...
(a) (10 pts) Use the following code to create a dataframe called `dat`. Then find the *mean* of the values columnwise. Use the function `map` or a function from it's family ONLY. ***Your final output should be a vector. Each element of the vector represents the mean values of each of the columns in dat.***
...
(b) (15pts) Using the dataset `airquality` in base R, find how many missing values are in each column. Use the function `map` or a function from it's family ONLY. To be able to answer this problem, you need to create a function such that the argument is a vector. The function should give you back the total number of missing values in that vector. 
...
***Your final output should be a vector. Each element of the vector represents the number of missing values for each column of the dataframe.***
