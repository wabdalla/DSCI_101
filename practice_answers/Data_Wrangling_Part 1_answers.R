...
# If you pull from the repository, inside the data folder, there is 
# a datafile called "Illustration_Data".
...
CIACountries <- read_csv("YOUR DIRECTORY HERE")

# 2. Take a look at the data with "slice": Display the first 7 rows
slice(CIACountries, 1:7)
...
question_13 <- CIACountries %>%
  select(2:8) %>%
  select(-area,-net_users)
