...
question_2 <- question_1 %>%
  mutate(new_date = format(Date, format = "%a, %b %d, %Y"))
...
question_3 <- question_1 %>%
  mutate(Week_Day = format(Date, format = "%A")) %>%
  group_by(Week_Day) %>%
  summarize(N_Flights_per_Day = n())

# Most flights: Monday - 50690
# Least flights: Saturday - 38720
