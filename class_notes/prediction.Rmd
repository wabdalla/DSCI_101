...
census <- read_csv(url, col_names = c("age", "workclass", "fnlwgt", "education", "education_1", "marital_status", "occupation", "relationship", "race", "sex", "capital_gain", "capital_loss", "hours_per_week", "native_country", "income"))%>%
...
pred <- train %>%
  ...
  bind_cols(predict(mod_null, new_data = train, type = "class")) %>%
...
```{r, warning=FALSE}
pred <- pred %>%
  bind_cols(predict(mod_log_1, new_data = train, type = "class")) %>%
