...
A **sampling distribution** is a distribution of a sample statistic based on all possible simple random samples of the same size from the same population. 
...
library(tidyverse)
mean_values <- map_dbl(1:10000, getxbar)


mean(mean_values)
sd(mean_values)

ggplot(tibble(mean_values), aes(mean_values))+
  geom_histogram()
...
For instance, the mean of all the sample means (`mean(mean_values)`) should be close to the true population mean (0 in this case), and the standard deviation (`sd(mean_values)`) gives us an idea of how much the sample means vary from one another and from the true population mean. This value is the standard error. This is essential for making statistical inferences based on our samples.
...
In this code, we use bootstrapping to estimate the standard error of the sample mean (`xbar`). We resample from our sample (`sample_dist`) with replacement to create a distribution of sample means. The standard deviation of these means provides an estimate of the standard error.
...
## Statistical Models
Statistical models are structures that allow us to relate variables to one another. They are used for various purposes, including prediction, inference, and explanation. It's important to remember that no model perfectly represents reality, but some models can be highly useful.
