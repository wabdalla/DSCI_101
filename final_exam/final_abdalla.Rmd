...
d. Create a new column called `month` based on the `date` variable. The month column should contain the actual names of the month in a truncated form (i.e., Jan, Feb... etc), then determine the ***max*** total number of cases per month. **Your final output should be a printed dataframe with 12 rows and two columns: the month and the max number of cases per month.**
...
Using `plotly`, generate a scatter plot for Total (y axis) vs Male (x axis). In the same plot, separate the data points by Day of the Week (Sat, Sun, Mon,...etc.), where each day is a different color. The plot title should be "Covid Cases: Total Cases vs Male Cases by Day of the Week", the x-axis title should be "Male Cases", and the y-axis title should be "Total Cases". **Your final output should be a printed interactive plot.**
...
Take the male cases in the year 2020 as a sample and perform a bootstrap analysis. Estimate the standard error of the **mean**. Perform 1000 iterations.
...
Suppose a vending machine has different kinds of candy as follows: 4 Snickers, 2 Twix, and 4 Skittles. If you randomly draw one of those candy, the theoretical probability that the candy is Skittles is 4/10 (0.4). 

Create a function that simulates drawing one candy from that vending machine. The arguments of the function should be `i`, `vending_machine`, and `candy`, representing the iterations, the vector of all candies in the vending machine and the kind of candy you want to compute the probability on, respectively. 

Use this function to simulate the experimental probability of getting a Skittles. Use 1000 iterations. **Your final output should be the printed experimental probability of getting a Skittles.**
