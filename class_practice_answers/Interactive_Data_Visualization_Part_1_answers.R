...
plot_1<-plot_ly(data = SAT_2010, x = ~read, y = ~math, 
                color = ~state,
             type = "scatter", mode = "markers") %>%
  layout(title="Math vs Read per State", 
...
plot_2<-plot_ly(data = SAT_2010, x = ~read, y = ~math,
                type = "scatter", mode = "markers", 
                marker = list(size = 10,
                              color = "black",
...
plot_3<-plot_ly(data = Salaries, x = ~read, y = ~math,
                color = ~salary_level,
                type = "scatter", mode = "markers", 
                colors = c("deepskyblue", "hotpink", "orangered"))
plot_3

# 4. Build a scatter plot with three variables and the following symbols: 
#    "circles", "triangles", "squares"
plot_4<-plot_ly(data = Salaries, x = ~read, y = ~math,
                color = ~salary_level,
                symbol = ~salary_level,
                type = "scatter", mode = "markers", 
                colors = c("deepskyblue", "hotpink", "orangered"))
...
plot_5 <- plot_ly(data = SAT_2010, x = ~state, y = ~salary, 
                     type = "scatter", mode = "lines") %>%
  layout(title="Salary per State", 
         plot_bgcolor="aliceblue", 
         xaxis = list( 
           title = "State",
           gridcolor = "white"), 
...
plot_6 <- plot_ly(data = SAT_2010, x = ~state, y = ~read, name = "Read", 
                     type = "scatter", mode = "lines+markers",
                     line = list(color = "red", width = 4))%>%
  add_trace(y = ~math, name = "Math", 
            type = "scatter", mode = "lines",
            line = list(color = "blue", width = 4, dash = "dot")) %>%
  add_trace(y = ~write, name = "Write", 
            type = "scatter", mode = "lines",
            line = list(color = "green", width = 4, dash = "dash")) %>%
  layout(title = "State vs. SAT Scores",
