...
question_2a <- SAT_2010 %>% 
  ggplot(aes(x = read)) + 
  geom_density(bw = 10)
question_2a

question_2b <- SAT_2010 %>% 
  ggplot(aes(x = read)) + 
  geom_density(bw = 5)
question_2b
...
question_7 <- SAT_More_Info %>% 
  ggplot(aes(x = psr_cat)) +
  geom_bar()
question_7

# 8. Create a bar graph such that psr low comes first and psr high comes second.

question_8 <- SAT_More_Info %>% 
  mutate(psr_cat = factor(psr_cat, levels = c("psr - low", "psr - high"))) %>% 
  ggplot(aes(x = psr_cat)) +
  geom_bar()
question_8
