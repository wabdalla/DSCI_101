...
# Your answer: left them at the end. 
...
question_4 <- CIACountries%>%
  arrange(net_users, educ) %>%
  select(net_users, educ)
...
question_8 <- CIACountries %>%
  mutate(area_category = case_when(
    area<2498 ~ "small",
    ...
    TRUE ~ "no information"))%>%
  filter(net_users == ">60%") %>%
  select(country, pop, net_users, area_category) %>%
  arrange(desc(pop))
