...
# Exercise 5: Generalize the function you created on exercise 4 even further. 
#             This time, let the arguments be i, urn, and any outcome. 
#             So that the function works for any urn, and tests for any outcome. 
#             Test your function with an urn of different colors and your choice
#             of an outcome. 
#             Compute experimental probabilities each one of the colors. 
#             Compare your experimental probability to the theoretical probability. 


marble_function_general_urn <- function(i, urn, outcome){
  drawing_a_marble <- sample( urn, 1)
  check_marble <- drawing_a_marble == outcome
  return(check_marble)
}

new_urn<- c("purple", "purple", "blue", "blue", "blue")

iterations <- map_dbl(1:100000, marble_function_general_urn, urn = new_urn,  outcome = "purple")
experimental_prob <- sum(iterations)/length(iterations)
experimental_prob

### Theoretical probability

theoretical_prob <- 2/5
...
# Exercise 6: Calculate the probability of getting any side of any kind 
#             of dice. 
#             In order to achieve this, create a function such that the 
#             arguments are i, dice, and any outcome. 
#             This function should work for any dice, and tests for any 
#             outcome. 
#             Test your function with a dice that has 10 sides, 
#             and compute the probability of getting the side of 6. 
...
dice_function_general <- function(i, dice, outcome){
  rolling_dice <- sample(dice, 1)
  check_dice <- rolling_dice == outcome
  return(check_dice)
}

ten_sided_die<- 1:10

iterations <- map_dbl(1:100000, dice_function_general, dice = ten_sided_die,  outcome = 6)
experimental_prob <- sum(iterations)/length(iterations)
experimental_prob
